...
def multiplica_matrices(A: np.ndarray, B: np.ndarray) ->np.ndarray:
    ...
    C = [[0 for _ in range(m)] for _ in range(p)]
...
def genera_matriz_aleatoria(n) -> np.ndarray:
    """
    Genera una matriz aleatoria de tamaño n x n.
    """
    return np.random.random((n, n))

def tarea_matriz(n: int) -> None:
    """
    Genera dos matrices aleatorias de tamano n x n y las multiplica.
    """
    A = genera_matriz_aleatoria(n)
    B = genera_matriz_aleatoria(n)
    multiplica_matrices(A, B)

def tarea1(size: int) -> None:
    """
    Proceso que ejecuta solo la tarea más costosa (400x400).
    """
    tarea_matriz(size)

def tarea2(sizes: List[int]) -> None:
    """
    Proceso que ejecuta de forma secuencial varias tareas de multiplicación.
    """
    for s in sizes:
        tarea_matriz(s)
...
def compare_execution() -> Tuple[float, float]:
    ...
    tamano1 = 400
    tamano2 = [s for s in matrix_sizes if s != tamano1]

    p1 = mp.Process(target=tarea1, args=(tamano1,))
    p2 = mp.Process(target=tarea2, args=(tamano2,))
